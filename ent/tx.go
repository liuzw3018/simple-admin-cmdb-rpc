...
func (tx *Tx) init() {
	tx.Cmdb = NewCmdbClient(tx.config)
	tx.CmdbAssets = NewCmdbAssetsClient(tx.config)
	tx.CmdbIp = NewCmdbIpClient(tx.config)
	tx.CmdbIpUser = NewCmdbIpUserClient(tx.config)
}
...
// applies a query, for example: Cmdb.QueryXXX(), the query will be executed
